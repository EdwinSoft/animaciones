...
class BiseccionAnimacion(MovingCameraScene):
    def construct(self):
        ...
        bis = Biseccion(f, 2, 3, 0.001, tipo_error='t')
        ...
        graph = axes.plot(f, color=BLUE, x_range=[0, 3])
        graph_label = axes.get_graph_label(graph, label=r"8-4.5(x-\sin(x))", x_val=0.5, buff=2, direction=UR).scale(0.6)
        ...
        for i in range(10):
            ...
            line_a = axes.get_vertical_line(axes.c2p(a, f(a)), color=YELLOW, stroke_width=2*escala,line_config={"dash_length": 0.05 * escala})
            #line_a = line_a.to_dashed(dash_length=0.15, spacing=0.1)
            line_b = axes.get_vertical_line(axes.c2p(b, f(b)), color=YELLOW, stroke_width=2*escala,line_config={"dash_length": 0.05 * escala})
            ...
            label_b = MathTex(f"b_{i}").scale(0.6*escala).next_to(dot_b, DOWN*escala, buff=0.05)
            ...
            self.play(
                ...
                FadeOut(label_b), FadeOut(label_m), run_time=5.5
            )
